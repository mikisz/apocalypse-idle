...
describe('settlers tick', () => {
  it('keeps potato totals consistent with farming bonus and consumption', () => {
    ...
    state.population.settlers = [
      createSettler({
        id: 's1',
        role: 'farmer',
        skills: { farmer: { level: 1 } },
      }),
    ];

    const roleBonuses: Record<string, number> = computeRoleBonuses(
      state.population.settlers,
    );
    const productionBonuses: Record<string, number> = { ...roleBonuses };
  ...
  it('clamps food once after bonus and consumption', () => {
    ...
    state.population.settlers = [createSettler({ id: 's1' })];
